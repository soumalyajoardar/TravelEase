...
import { getAdminOffers, Offer } from '@/services/offerService';
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 p-4 md:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-primary">Offers & Deals</h1>
          <p className="text-secondary mt-1">Manage promotional offers and coupon codes.</p>
        </div>
        <button className="flex items-center justify-center space-x-2 bg-primary text-white px-4 py-2 rounded text-sm font-medium hover:bg-opacity-90 transition-opacity">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-border overflow-hidden">
        <div className="p-4 border-b border-border bg-gray-50 flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4">
          ...
          <select disabled={isLoading || offers.length === 0} className="w-full sm:w-auto border border-border rounded px-3 py-2 text-sm bg-white text-secondary">
        ...
        <div className="overflow-x-auto min-h-[400px] flex flex-col">
          {isLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center p-8">
              <AdminEmptyState 
                icon={Tag}
                title="No offers have been created yet."
                description="Create promotional offers and discounts to provide savings for customers."
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50 border-b border-border text-secondary">
                <tr>
                  ...
                  <th className="px-6 py-3 font-medium uppercase tracking-wider text-xs">Discount</th>
                  <th className="px-6 py-3 font-medium uppercase tracking-wider text-xs">Validity</th>
              ...
              <tbody>
                {/* Simulated mapped rows would go here */}
              </tbody>
            </table>
          )}
        </div>
