...
import { getBusServices } from '@/services/supabaseAdminService';
...
export default function AdminBusServicesPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await getBusServices();
        setServices(data);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="max-w-6xl mx-auto space-y-6 p-4 md:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-secondary mt-1">Manage bus inventory and vehicle types.</p>
        </div>
        ...
        <div className="overflow-x-auto min-h-[400px] flex flex-col">
          {isLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center p-8">
              <AdminEmptyState 
                icon={Bus}
                title="No bus services have been added yet."
                description="Add bus services and link them to their respective operators."
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50 border-b border-border text-secondary">
                <tr>
                  ...
                  <th className="px-6 py-3 font-medium uppercase tracking-wider text-xs">Vehicle Type</th>
