...
import { Train, Plus, Search } from 'lucide-react';
import { getTrainServices } from '@/services/supabaseAdminService';
...
export default function AdminTrainServicesPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const data = await getTrainServices();
        setServices(data);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    <div className="max-w-6xl mx-auto space-y-6 p-4 md:p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-secondary mt-1">Manage train inventory and numbers.</p>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-border overflow-hidden">
        <div className="p-4 border-b border-border bg-gray-50 flex items-center space-x-4">
          ...
          <select disabled={isLoading || services.length === 0} className="border border-border rounded px-3 py-2 text-sm bg-white text-secondary hidden sm:block">
        ...
        <div className="overflow-x-auto min-h-[400px] flex flex-col">
          {isLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center p-8">
              <AdminEmptyState 
                icon={Train}
                title="No train services have been added yet."
                description="Add train services and link them to their respective operators."
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50 border-b border-border text-secondary">
                <tr>
                  <th className="px-6 py-3 font-medium uppercase tracking-wider text-xs">Train Number</th>
                  <th className="px-6 py-3 font-medium uppercase tracking-wider text-xs">Train Name</th>
